...
struct DeckCreatureView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            let cardWidth = width * (0.77 + growth * 0.08)
            let cardHeight = height * (0.61 + growth * 0.05)
            let layerSpacing = 1.8 + growth * 1.7
            let stackDepth = CGFloat(max(visibleLayers - 1, 0)) * layerSpacing
            let centerY = height / 2 + stackDepth / 2
            let fanAmount = 0.65 + growth * 0.35

            ZStack {
                Ellipse()
                    .fill(appearance.palette.ink.color.opacity(0.10))
                    .frame(width: cardWidth * 0.70, height: 4)
                    .blur(radius: 2)
                    .position(x: width / 2, y: centerY + cardHeight / 2 + 3)

                ForEach(Array((0..<visibleLayers).reversed()), id: \.self) { layer in
                    creatureCard(layer: layer, width: cardWidth, height: cardHeight, fanAmount: fanAmount)
                        .position(
                            x: width / 2 + appearance.layerOffsets[layer] * fanAmount,
                            y: centerY - CGFloat(layer) * layerSpacing
                        )
                }
            }
    ...
    @ViewBuilder
    private func creatureCard(layer: Int, width: CGFloat, height: CGFloat, fanAmount: Double) -> some View {
        let isFront = layer == 0
        ...
        .rotationEffect(.degrees(appearance.layerTilts[layer] * fanAmount))
    }

    private func frontCardDetails(width: CGFloat, height: CGFloat) -> some View {
        ZStack {
            if deckCreaturesHaveFaces {
            ...
            HStack(spacing: 4) {
                ForEach(0..<detailLineCount, id: \.self) { _ in
                    Capsule()
...
private struct CardCreatureFace: View {
    ...
    var body: some View {
        Canvas { context, size in
            let eyeSize = CGSize(width: 13, height: 16)
            let eyeDistance = min(appearance.eyeSpacing + eyeSize.width, size.width - eyeSize.width - 4)
            let eyeY = size.height * 0.31 + appearance.eyeHeight

            for direction in [-1.0, 1.0] {
                let centerX = size.width / 2 + direction * eyeDistance / 2
                let eyeRect = CGRect(
                    x: centerX - eyeSize.width / 2,
                    y: eyeY - eyeSize.height / 2,
                    width: eyeSize.width,
                    height: eyeSize.height
                )
                context.fill(Path(roundedRect: eyeRect, cornerRadius: eyeSize.width / 2), with: .color(Color(red: 0.99, green: 0.99, blue: 0.97)))

                let pupilSize = 6.2 * appearance.pupilScale
                let pupilRect = CGRect(
                    x: centerX - pupilSize / 2 + appearance.pupilOffset,
                    y: eyeY - pupilSize / 2 + 1,
                    width: pupilSize,
                    height: pupilSize
                )
                context.fill(Path(ellipseIn: pupilRect), with: .color(appearance.palette.ink.color))

                let shineRect = CGRect(x: pupilRect.maxX - 2.1, y: pupilRect.minY, width: 2.1, height: 2.1)
                context.fill(Path(ellipseIn: shineRect), with: .color(Color(red: 1, green: 1, blue: 0.98)))
            }

            let mouthWidth = appearance.mouthStyle == .tinySmile ? 9.0 : 14.0
            let mouthRect = CGRect(x: size.width / 2 - mouthWidth / 2, y: size.height * 0.66, width: mouthWidth, height: 7)
            if appearance.mouthStyle == .openSmile {
                context.fill(Path(roundedRect: mouthRect, cornerRadius: 3.5), with: .color(appearance.palette.ink.color))
                let tongueRect = CGRect(x: mouthRect.minX + 2, y: mouthRect.maxY - 3.5, width: mouthRect.width - 4, height: 2.5)
                context.fill(Path(roundedRect: tongueRect, cornerRadius: 1.25), with: .color(appearance.palette.accent.color))
            } else {
                var smile = Path()
                smile.move(to: CGPoint(x: mouthRect.minX, y: mouthRect.minY + mouthRect.height * 0.25))
                smile.addQuadCurve(
                    to: CGPoint(x: mouthRect.maxX, y: mouthRect.minY + mouthRect.height * 0.25),
                    control: CGPoint(x: mouthRect.midX, y: mouthRect.maxY)
                )
                context.stroke(smile, with: .color(appearance.palette.ink.color), style: StrokeStyle(lineWidth: 2, lineCap: .round))
            }
        }
    }
    ...
    private var eye: some View {
        ZStack {
            Capsule()
                .fill(Color(red: 0.99, green: 0.99, blue: 0.97))
            ...
            Circle()
                ...
                .overlay(alignment: .topTrailing) {
                    Circle()
                        .fill(Color(red: 1, green: 1, blue: 0.98))
